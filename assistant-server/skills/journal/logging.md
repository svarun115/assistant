...
**Before asking for details**, gather surrounding context:

Journal entries nearby:
```
query(entity="journal_entries", where={"date": {"gte": "TARGET_DATE-3", "lte": "TARGET_DATE+3"}})
```

Events nearby:
```
query(entity="events", where={"date": {"gte": "TARGET_DATE-2", "lte": "TARGET_DATE+2"}}, orderBy="start", orderDir="asc")
```

Also fetch Garmin activities and sleep locations for the missing date.
