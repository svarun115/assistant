...
```
REFRESH only runs: today's events (Query 1) + Garmin BB update. Entity cache comes from context — no DB queries for entity resolution.
...
If the user describes past activities: load `logging.md` and delegate to journal agent (LOG-ONLY mode, background). Acknowledge immediately. Check output before next response.
