...
## Step 1: Read Plan + Fetch Today's Events in Parallel
...
```
REFRESH only runs: today's events (Query 1) + Garmin final stats. Entity cache comes from context.

**1d. Garmin:** `get_user_summary(date: "<today>")` — final step count, Body Battery end

Run 1c + 1d in parallel (separate MCP blocks per MCP isolation rules). 1a + 1b are local file reads.

## Step 2: Build Plan vs. Actual

Using the plan's Timeline and the events returned from the journal agent:
...
4. Calculate:
   ...
   - Unlogged gaps: periods > 30 min with no logged event
...
Wait for user input. Log anything reported via journal agent (LOG-ONLY, background). Acknowledge immediately.
...
Log one `journal_entries` entry via journal agent (LOG-ONLY mode):
