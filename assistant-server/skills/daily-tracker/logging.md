...
**All journal writes go through the journal agent.** Never call `mcp__personal-journal__*` directly. The journal agent has gotchas.md and entity resolution context loaded — direct calls fail on field formats and miss entity linking.

Resume the journal agent in LOG-ONLY mode:

```
mode: "LOG-ONLY"
entry_date: "<today>"
cache_path: "~/.claude/data/daily-context.json"
events: [
  {
